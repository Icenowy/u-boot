...
void mctl_set_timing_params(uint16_t socid, struct dram_para *para)
{
	...
	u8 tcksre	= 5;
	u8 tckesr	= 4;
	u8 trasmax	= CONFIG_DRAM_CLK / 30;

	u8 tcl		= (CONFIG_DRAM_CLK >= 410) ? 4 : 3; /* CL 6 or 8 */
	u8 tcwl		= 3; /* CWL 6 */
	u8 t_rdata_en	= (CONFIG_DRAM_CLK >= 410) ? 2 : 1;
	u8 wr_latency	= 1;
	...
	/* set mode register */
	writel((CONFIG_DRAM_CLK >= 410) ? 0xe73 : 0xa63, &mctl_ctl->mr[0]);
	...
	/* set two rank timing */
	clrsetbits_le32(&mctl_ctl->dramtmg[8], (0xff << 24) | (0xff << 8) |
					       (0xff << 0),
